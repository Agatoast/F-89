...
namespace F89.UI
{
    public static class MissingEquipmentDialog
    {
        public enum Result
        {
            None = 0,
        ...
        public static Result Draw(bool visible, string missingTypesCsv)
        {
            if (!visible)
            {
                return Result.None;
            ...
            var message = string.IsNullOrWhiteSpace(missingTypesCsv)
                ? "Missing Equipment"
                : $"Missing Equipment: {missingTypesCsv}";
            GUI.Label(
                new Rect(dialogRect.x + 24f, dialogRect.y + 24f, dialogRect.width - 48f, 80f),
                message,
                messageStyle);
