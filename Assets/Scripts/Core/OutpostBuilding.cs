...
namespace F89.Core
{
    public sealed class OutpostBuilding : MonoBehaviour
    {
        ...
        public void MarkDestroyed()
        {
            ...
            PersistDestroyedBuilding();
            GroundExplosionEffect.PlayBuildingExplosion(transform.position, buildingType.ToString());
            RevealSurfaceBunkerPadIfNeeded();
            gameObject.SetActive(false);
        }

        private void PersistDestroyedBuilding()
        {
            ...
            var lockable = GetComponent<LockableTarget>();
            var label = lockable != null ? lockable.TargetLabel : buildingType.ToString();
            AntarcticaOutpostState.MarkTargetDestroyed(baseSite.BaseName, label);
        }
        ...
        private void RevealSurfaceBunkerPadIfNeeded()
        {
            if (buildingType != OutpostBuildingType.Bunker)
            {
                return;
            }
